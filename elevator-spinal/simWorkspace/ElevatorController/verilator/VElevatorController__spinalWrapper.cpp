...
class Wrapper_9138091360174570420;
thread_local Wrapper_9138091360174570420 *simHandle9138091360174570420;
...
public:
    ...
    Wrapper_9138091360174570420(const char * name, const char * wavePath, int seed){
      //contextp = new VerilatedContext;
      Verilated::randReset(2);
      Verilated::randSeed(seed);
      top = new VElevatorController();

      simHandle9138091360174570420 = this;
      time = 0;
      timeCheck = 0;
      lastFlushAt = high_resolution_clock::now();
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top->_zz_when_ElevatorController_l44 );
      signalAccess[1] = new CDataSignalAccess( top->_zz_when_ElevatorController_l51 );
      signalAccess[2] = new CDataSignalAccess( top->when_ElevatorController_l102 );
      signalAccess[3] = new CDataSignalAccess( top->when_ElevatorController_l57 );
      signalAccess[4] = new CDataSignalAccess( top->_zz_1 );
      signalAccess[5] = new CDataSignalAccess( top->_zz_2 );
      signalAccess[6] = new CDataSignalAccess( top->_zz_3 );
      signalAccess[7] = new CDataSignalAccess( top->_zz_4 );
      signalAccess[8] = new CDataSignalAccess( top->_zz_5 );
      signalAccess[9] = new CDataSignalAccess( top->_zz_6 );
    ...
    virtual ~Wrapper_9138091360174570420(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/home/fabio_testino/elevator-spinal/./simWorkspace/ElevatorController/$TEST/") + name + ".dat").c_str());
      #endif
...
double sc_time_stamp () {
  return simHandle9138091360174570420->time;
}
...
JNIEXPORT Wrapper_9138091360174570420 * API JNICALL Java_wrapper_1verilator_VerilatorNative_newHandle_19138091360174570420
  (JNIEnv * env, jobject obj, jstring name, jstring wavePath, jint seedValue){
    ...
    Wrapper_9138091360174570420 *handle = new Wrapper_9138091360174570420(ch, wavePathCh, seedValue);
    env->ReleaseStringUTFChars(name, ch);
    env->ReleaseStringUTFChars(wavePath, wavePathCh);
    return handle;
}

JNIEXPORT jboolean API JNICALL Java_wrapper_1verilator_VerilatorNative_eval_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle){
   handle->top->eval();
   return Verilated::gotFinish();
}

JNIEXPORT jint API JNICALL Java_wrapper_1verilator_VerilatorNative_getTimePrecision_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle){
  return handle->time_precision;
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_sleep_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle, uint64_t cycles){
  #ifdef TRACE
...
JNIEXPORT jlong API JNICALL Java_wrapper_1verilator_VerilatorNative_getU64_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle, int id){
  return handle->signalAccess[id]->getU64();
}

JNIEXPORT jlong API JNICALL Java_wrapper_1verilator_VerilatorNative_getU64mem_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle, int id, uint64_t index){
  return handle->signalAccess[id]->getU64_mem(index);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_setU64_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle, int id, uint64_t value){
  handle->signalAccess[id]->setU64(value);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_setU64mem_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 *handle, int id, uint64_t value, uint64_t index){
  handle->signalAccess[id]->setU64_mem(value, index);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_deleteHandle_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 * handle){
  delete handle;
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_getAU8_19138091360174570420
  (JNIEnv * env, jobject obj, Wrapper_9138091360174570420 * handle, jint id, jbyteArray value){
  handle->signalAccess[id]->getAU8(env, value);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_getAU8mem_19138091360174570420
  (JNIEnv * env, jobject obj, Wrapper_9138091360174570420 * handle, jint id, jbyteArray value, uint64_t index){
  handle->signalAccess[id]->getAU8_mem(env, value, index);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_setAU8_19138091360174570420
  (JNIEnv * env, jobject obj, Wrapper_9138091360174570420 * handle, jint id, jbyteArray value, jint length){
  handle->signalAccess[id]->setAU8(env, value, length);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_setAU8mem_19138091360174570420
  (JNIEnv * env, jobject obj, Wrapper_9138091360174570420 * handle, jint id, jbyteArray value, jint length, uint64_t index){
  handle->signalAccess[id]->setAU8_mem(env, value, length, index);
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_enableWave_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 * handle){
  handle->waveEnabled = true;
}

JNIEXPORT void API JNICALL Java_wrapper_1verilator_VerilatorNative_disableWave_19138091360174570420
  (JNIEnv *, jobject, Wrapper_9138091360174570420 * handle){
